...
import { createAvatar, uploadAvatarImage } from "@/app/actions/anak-assets";
...
export function CreateAvatarForm() {
  ...
  const handleSave = () => {
    if (!name.trim()) {
      toast.error("Nama avatar wajib diisi.");
      return;
    }
    if (!imageUrl) {
    ...
    startSave(async () => {
      const res = await createAvatar({
        name: name.trim(),
        costume: costume.trim() || undefined,
        imageUrl,
        unlockLevel,
      });
      if (!res.success) {
        toast.error(res.error ?? "Gagal menyimpan avatar.");
        return;
      }
      toast.success("Avatar berhasil ditambahkan.");
      reset();
      router.refresh();
    });
  };
  ...
  if (!open) {
    return (
      <GameButton type="button" variant="accent" block onClick={() => setOpen(true)}>
        <Plus className="w-4 h-4" /> Tambah Avatar
      </GameButton>
    );
  }
  ...
  return (
    <div className="rounded-2xl border-2 border-border bg-card shadow-card p-4 space-y-3">
      <h2 className="font-heading font-extrabold text-ink-1">Tambah Avatar</h2>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-1">
          <Label>Nama Avatar</Label>
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Contoh: Daffa Astronot" />
        </div>
        <div className="space-y-1">
          <Label>Kostum (opsional)</Label>
          <Input value={costume} onChange={(e) => setCostume(e.target.value)} placeholder="Contoh: baju astronot" />
        </div>
      </div>
